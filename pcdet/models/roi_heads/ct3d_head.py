...
class CT3DHead(RoIHeadTemplate):
    def __init__(self, input_channels, model_cfg, voxel_size=None, point_cloud_range=None, num_class=1, backbone_channels=None):
        ...
        num_queries = model_cfg.Transformer.num_queries #1
        hidden_dim = model_cfg.Transformer.hidden_dim # 256
        self.num_points = model_cfg.Transformer.num_points # 256
        ...
        self.aux_loss = model_cfg.Transformer.aux_loss # False
    ...
    def forward(self, batch_dict):
        ...
        rois = batch_dict['rois'] # (b, nr=128, 7)
        ...
        for bs_idx in range(batch_size):
            ...
            for roi_box_idx in range(0, num_rois):
                ...
                if cur_roi_points.shape[0] >= num_sample:
                    random.seed(0)
                    index = np.random.randint(cur_roi_points.shape[0], size=num_sample) # replicate
        ...
        corner_points = corner_points.view(batch_size * num_rois, -1) # b*128, 24
        corner_add_center_points = torch.cat([corner_points, rois.view(-1, rois.shape[-1])[:,:3]], dim = -1) # 24+3 = 27
