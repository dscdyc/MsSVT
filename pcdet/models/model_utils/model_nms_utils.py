...
def class_agnostic_nms(box_scores, box_preds, nms_config, score_thresh=None, idx=None):
    ...
    if isinstance(nms_config.NMS_THRESH, list):
        if idx is not None:
            nms_thresh = nms_config.NMS_THRESH[idx]
        elif len(nms_config.NMS_THRESH) == 1:
            nms_thresh = nms_config.NMS_THRESH[0]
        else:
            raise ValueError('NMS_THRESH dose not match the given idx.')
    else:
